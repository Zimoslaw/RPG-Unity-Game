...
public class skills : MonoBehaviour {
	...
	public class melee : Component {
		...
		public void Cast(GameObject character, float range, GameObject visual, int aDmg, int mDmg) {
			...
			foreach(Collider x in colliders) {
				if(x.CompareTag("Alive")) {
					if(Vector3.Angle(character.transform.forward, x.transform.position) < 45) { //jeśli kąt pomiędzy obiektem a graczemjest mniejszy niż 45
						if (aDmg != 0)
